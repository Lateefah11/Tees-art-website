/* ============================================================
   GLSL HILLS — Vanilla JS / Three.js port
   Tee's Art · Hero Section Background
   ============================================================ */
...
(function () {
  ...
  /* ── Fragment Shader ─────────────────────────────────────── */
  // Dark burgundy tint for visibility on the light cream background
  const fragmentShader = `
    ...
    void main(void) {
      ...
      vec3 color = vec3(0.29, 0.12, 0.16);
  ...
  function initHills() {
    ...
    // Config (mirrors the React component defaults)
    const cameraZ   = 125;
    const planeSize = 256;
    const speed     = 0.5;

    // Create canvas and insert as the very first child (bottom layer)
    const canvas = document.createElement('canvas');
    canvas.id = 'hills-canvas';
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initHills);
  } else {
    initHills();
  }
